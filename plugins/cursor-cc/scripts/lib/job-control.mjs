...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting cursor") || line.startsWith("session ready") || line.startsWith("running cursor")) {
      return "starting";
    }
...
export function getSessionRuntimeStatus() {
  return {
    ...
    detail: "Runs are tracked by the Cursor ↔ Claude Code bridge (PID + log files). There is no shared app-server broker.",
    endpoint: null
  };
}
...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No run found for "${reference}". Run /cursor-cc:runs to list known runs.`);
}
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No run found for "${reference}". Run /cursor-cc:runs to inspect known runs.`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Run ${active.id} is still ${active.status}. Check /cursor-cc:runs and try again once it finishes.`);
  }

  if (reference) {
    throw new Error(`No finished run found for "${reference}". Run /cursor-cc:runs to inspect active runs.`);
  }

  throw new Error("No finished Cursor runs found for this repository yet.");
}
...
export function resolveCancelableJob(cwd, reference, options = {}) {
  ...
  if (sessionScopedActiveJobs.length > 1) {
    throw new Error("Multiple Cursor runs are active. Pass a run id to /cursor-cc:stop.");
  }

  if (getCurrentSessionId(options)) {
    throw new Error("No active Cursor runs to stop for this session.");
  }

  throw new Error("No active Cursor runs to stop.");
}
